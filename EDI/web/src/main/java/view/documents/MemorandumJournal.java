    ...
    private void setAttributesDependOnBookMark(HttpServletRequest req) {
        ...
        switch (bookMark1) {
            case "tasksListByGroup":
                req.setAttribute("tasksList", ExecutorTaskFolderStructureServiceImpl.INSTANCE.getTasksByFolder(currentUser, FolderStructure.valueOf(bookMark2), groupBy, mapSort.get(bookMark1), mapFilter.get(bookMark1)));
                req.setAttribute("mapSortValue", mapSort.get(bookMark1));
                break;
            case "fullTasksList":
                req.setAttribute("tasksList", ExecutorTaskFolderStructureServiceImpl.INSTANCE.getCommonList(currentUser,  mapSort.get(bookMark1), mapFilter.get(bookMark1)));
                req.setAttribute("mapSortValue", mapSort.get(bookMark1));
                break;
        }
